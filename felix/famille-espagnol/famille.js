...
import { renderFlashcard } from '../espagnol/games/flashcard.js';
import { renderTranslate } from '../espagnol/games/translate.js';
import { renderFillBlank } from '../espagnol/games/fill-blank.js';
import { renderMatchPairs } from '../espagnol/games/match-pairs.js';
import { renderListenType } from '../espagnol/games/listen-type.js';
import { renderSentenceBuilder } from '../espagnol/games/sentence-builder.js';
import { renderSpeakWord } from '../espagnol/games/speak-word.js';
import { renderSpeakTranslate } from '../espagnol/games/speak-translate.js';

const LEVELS = [
  { file: 'data/famille.json', key: 1 },
];
...
async function renderLevelGrid() {
  ...
  const allMastery = await DB.getAllMastery('familia');
...
async function startSession(levelKey) {
  ...
  sessionQueue = await buildSessionQueue('familia', data.items, 15);
...
async function renderGameItem() {
  ...
  const mastery = await getOrCreateMastery(item.id, 'familia', currentLevel);
...
async function handleAnswer(item, correct, container) {
  const mastery = await recordAnswer(item.id, correct, 'familia', currentLevel);
...
async function showSessionResults() {
  ...
  const lottieContainer = $('#es-results-lottie');
  if (pct >= 0.8) {
  ...
  $('#es-results-title').textContent = title;
  $('#es-results-score-num').textContent = sessionScore;
  $('#es-results-score-total').textContent = total;
  $('#es-results-message').textContent = message;

  $('#es-results-details').innerHTML = `
    <div class="es-result-detail"><span class="detail-num new-words">${newWordsLearned}</span><span class="detail-label">nouveaux mots</span></div>
    <div class="es-result-detail"><span class="detail-num reviewed">${reviewedWords}</span><span class="detail-label">révisés</span></div>
    <div class="es-result-detail"><span class="detail-num streak-num">${maxStreak}</span><span class="detail-label">meilleure série</span></div>
  `;
  ...
  try {
    ...
    $('#es-results-xp').textContent = `+${result.xp} XP gagnés !`;
    const levelItems = levelData[currentLevel]?.items || [];
    await checkLevelBadges('familia', currentLevel, levelItems);
  } catch (e) {
...
function bindNav() {
  ...
  $('#es-btn-retry').addEventListener('click', () => {
    SFX.play('tap');
    startSession(currentLevel);
  });

  $('#es-btn-levels').addEventListener('click', () => {
    SFX.play('tap');
    renderLevelGrid();
    showScreen('levels');
  });
...
/* ────────── TTS (Web Speech API — Spanish voice) ────────── */
...
function pickSpanishVoice(voices) {
  ...
  spanishVoice =
    esVoices.find(v => v.lang === 'es-ES' && v.name.toLowerCase().includes('monica') && v.localService) ||
    esVoices.find(v => v.lang === 'es-ES' && v.name.toLowerCase().includes('jorge') && v.localService) ||
    esVoices.find(v => v.lang === 'es-ES' && v.localService) ||
    esVoices.find(v => v.lang === 'es-MX' && v.localService) ||
    esVoices.find(v => v.name.toLowerCase().includes('enhanced')) ||
    esVoices.find(v => v.localService) ||
    esVoices[0] ||
    null;

  if (spanishVoice) console.log('Spanish voice:', spanishVoice.name, spanishVoice.lang);
}

speechSynthesis.onvoiceschanged = () => {
  pickSpanishVoice(speechSynthesis.getVoices());
};
...
function speak(text, lang = 'es-ES') {
  ...
  speechSynthesis.cancel();
  if (!spanishVoice) initSpanishVoice();
