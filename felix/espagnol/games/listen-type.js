...
export function renderListenType({ item, container, onAnswer, speak }) {
  ...
  container.innerHTML = `
    <div class="listen-container">
      ...
      <div class="listen-hint" id="lt-hint">${item.type === 'sentence' ? 'Une phrase en espagnol' : 'Un mot en espagnol'}</div>
  ...
  function check() {
    ...
    const correct = checkListening(userVal, item.es);
    ...
    if (correct) {
      ...
      feedback.innerHTML = `<div class="translate-correct-answer" style="color:var(--success)">✓ ${item.es}</div>`;
    } else {
      SFX.play('wrong');
