...
async function renderLevelGrid() {
  ...
  const allMastery = await DB.getAllMastery('espagnol');
...
async function startSession(levelKey) {
  ...
  sessionQueue = await buildSessionQueue('espagnol', data.items, 15);
...
async function renderGameItem() {
  ...
  const mastery = await getOrCreateMastery(item.id, 'espagnol', currentLevel);
...
async function handleAnswer(item, correct, container) {
  const mastery = await recordAnswer(item.id, correct, 'espagnol', currentLevel);
...
async function showSessionResults() {
  ...
  try {
    ...
    await checkLevelBadges('espagnol', currentLevel, levelItems);
  } catch (e) {
