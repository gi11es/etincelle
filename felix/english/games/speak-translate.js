...
export function renderSpeakTranslate({ item, container, onAnswer, speak }) {
  ...
  container.innerHTML = `
    <div class="speak-container">
      <div class="question-text">Comment dit-on en anglais :</div>
  ...
  // Start loading Whisper in background if not ready
  if (!STT.isReady && !STT.isLoading) {
    STT.init('english', (progress) => {
  ...
  skipBtn.addEventListener('click', () => {
    ...
    `;
    feedbackEl.innerHTML += `<div class="flashcard-example" style="margin-top:12px">${item.example_en || ''}</div>`;
  ...
  function processResult(said) {
    ...
    const correct = checkSpeech(said, item.en);
    ...
    if (correct) {
      ...
      feedbackEl.innerHTML = `
        <div class="translate-correct-answer" style="color:var(--success)">✓ ${item.en}</div>
