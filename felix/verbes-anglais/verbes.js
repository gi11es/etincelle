...
// Import game modules (verb-specific)
import { renderConjugation } from './games/conjugation.js';
import { renderVerbChoice } from './games/choice.js';
import { renderVerbRecognition } from './games/recognition.js';

const LEVELS = [
  { file: 'data/verbes-irreguliers.json', key: 1 },
];
...
// Pick a game based on mastery: new → recognition, learning → choice, mastered → conjugation.
// A little randomization keeps sessions varied.
function pickVerbGame(status) {
  if (!status || status === 'new') {
    return Math.random() < 0.75 ? 'recognition' : 'choice';
  } else if (status === 'learning') {
    return Math.random() < 0.6 ? 'choice' : 'conjugation';
  }
  return Math.random() < 0.75 ? 'conjugation' : 'choice';
}
...
async function renderLevelGrid() {
  ...
  const allMastery = await DB.getAllMastery('verbs-en');
...
async function startSession(levelKey) {
  ...
  sessionQueue = await buildSessionQueue('verbs-en', data.items, 15);
...
async function renderGameItem() {
  ...
  const mastery = await getOrCreateMastery(item.id, 'verbs-en', currentLevel);

  // Pick game type based on mastery
  const gameType = pickVerbGame(mastery.status);
  ...
  switch (gameType) {
    case 'recognition': renderVerbRecognition(ctx); break;
    case 'choice': renderVerbChoice(ctx); break;
    case 'conjugation': renderConjugation(ctx); break;
    default: renderVerbRecognition(ctx);
  }
}

async function handleAnswer(item, correct, container) {
  const mastery = await recordAnswer(item.id, correct, 'verbs-en', currentLevel);
...
async function showSessionResults() {
  ...
  $('#en-results-details').innerHTML = `
    <div class="en-result-detail"><span class="detail-num new-words">${newWordsLearned}</span><span class="detail-label">nouveaux verbes</span></div>
  ...
  try {
    ...
    await checkLevelBadges('verbs-en', currentLevel, levelItems);
  } catch (e) {
