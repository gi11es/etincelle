...
export function renderListenType({ item, container, onAnswer, speak }) {
  ...
  container.innerHTML = `
    <div class="listen-container">
      ...
      <div class="listen-hint" id="lt-hint">Un mot ou une expression en français</div>
  ...
  function playAudio() {
    ...
    if (playCount >= 2) {
      ...
      hint.textContent = `Indice : ${item.en}`;
    }
  ...
  function check() {
    ...
    const correct = checkListening(userVal, item.fr);
    ...
    feedback.innerHTML += `<div class="flashcard-example" style="margin-top:12px">${item.en}</div>`;
